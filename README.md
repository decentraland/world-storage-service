...
- **Environment Variables Management**: Serves encrypted environment variables (secrets, API keys, config) configured at deploy time. GET value reads are restricted to authorized addresses (AUTHORITATIVE_SERVER_ADDRESS) or an authoritative scene worker presenting a valid scene-scoped storage delegation for its own scene, while write/delete operations are restricted to world owners and deployers only. This lets a headless authoritative worker consume its own scene's secrets (confined to its place) while keeping management in the owners' hands.
...
- **Collaborator Access & Discovery**: Wallets listed in a scene's `logsPermissions` metadata are granted read/write/delete on that scene's Scene and Player storage (alongside the world owner and deployers), re-checked on every request against scene metadata cached for a short TTL (default 30s) — so a revoked wallet loses access within that window. A deployment-fed, eventually-consistent index (`scene_collaborators`, kept current by an SQS consumer) powers the self-scoped `GET /collaborator` endpoint, which lets a wallet discover the scenes it collaborates on (identifiers only, never storage values). The index is a best-effort discovery hint, not a source of truth; see `docs/database-schemas.md` for its staleness model.
