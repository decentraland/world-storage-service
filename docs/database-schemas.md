...
- **Discovery only, eventually consistent**: populated from deployment events (SQS) and a write-through on first authorized access. It is not a source of truth — every storage endpoint re-verifies `logsPermissions` live.
- **Latest deployment wins**: a destructive replacement at a `(world_name, base_parcel)` location is skipped when an indexed row there has a higher `deployed_at`, so out-of-order SQS delivery cannot resurrect a stale scene.
